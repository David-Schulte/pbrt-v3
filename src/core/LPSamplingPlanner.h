...
namespace pbrt
{
	...
	private:
		...
		int findMinErrorLinModelIdx(std::vector<LinearModel> linModels, Float minErrorThreshold = 0.001);
		int64_t getPlannedSampleNumber(LinearModel minErrorLinModel, int64_t additionalSampleStep, Float invMinErrorThresholdFactor = 1000.0);		//Debug! Currently test implementation for matrix inverse computation
