...
// ----------------------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------------------
        ...
        template <typename T, long NR, long NCDLIB, typename MM, typename layout>
        void fft1d_inplace(matrix<std::complex<T>,NR,NCDLIB,MM,layout>& data, bool do_backward_fft, twiddles<T>& cs)
        /*!
            requires
                - is_vector(data) == true
    ...
    // ------------------------------------------------------------------------------------
        ...
        void fft2d_inplace(
            matrix<std::complex<T>,NR,NCDLIB,MM,L>& data,
...
// ----------------------------------------------------------------------------------------
    ...
    template < typename T, long NR, long NCDLIB, typename MM, typename L >
    void fft_inplace (matrix<std::complex<T>,NR,NCDLIB,MM,L>& data)
    ...
    template < typename T, long NR, long NCDLIB, typename MM, typename L >
    void ifft_inplace (matrix<std::complex<T>,NR,NCDLIB,MM,L>& data)
    {
        // make sure requires clause is not broken
        DLIB_CASSERT(is_power_of_two(data.nr()) && is_power_of_two(data.nc()),
...
// ----------------------------------------------------------------------------------------
    ...
    */
    ...
    template <long NR, long NCDLIB, typename MM, typename L>
    matrix<std::complex<double>,NR,NCDLIB,MM,L> call_fftw_fft(
        const matrix<std::complex<double>,NR,NCDLIB,MM,L>& data
    )
    {
        // make sure requires clause is not broken
        DLIB_CASSERT(is_power_of_two(data.nr()) && is_power_of_two(data.nc()),
        ...
        matrix<std::complex<double>,NR,NCDLIB,MM,L> m2(data.nr(),data.nc());
        fftw_complex *in, *out;
        fftw_plan p;
    ...
    template <long NR, long NCDLIB, typename MM, typename L>
    matrix<std::complex<double>,NR,NCDLIB,MM,L> call_fftw_ifft(
        const matrix<std::complex<double>,NR,NCDLIB,MM,L>& data
    )
    {
        // make sure requires clause is not broken
...
#define DLIB_DFTI_CHECK_STATUS(s) \
    ...
    template < long NR, long NCDLIB, typename MM, typename L >
    matrix<std::complex<double>,NR,NCDLIB,MM,L> call_mkl_fft(
        const matrix<std::complex<double>,NR,NCDLIB,MM,L>& data,
        bool do_backward_fft)
    {
        // make sure requires clause is not broken
        ...
        matrix<std::complex<double>,NR,NCDLIB,MM,L> out(data.nr(), data.nc());
    ...
    void call_mkl_fft_inplace(
        matrix<std::complex<double>,NR,NCDLIB,MM,L>& data,
        bool do_backward_fft
    )
    {
        // make sure requires clause is not broken
