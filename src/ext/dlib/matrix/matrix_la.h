...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        const long NCDLIB = matrix_exp<EXP>::NCDLIB;
        ...
        COMPILE_TIME_ASSERT(NCDLIB == 0 || vN == 0 || NCDLIB == vN);
...
// ----------------------------------------------------------------------------------------
    ...
    {
        qr_decomposition<matrix<T,NR,NCDLIB,MM,L> >(m).get_q(m);
    }
...
// ----------------------------------------------------------------------------------------
    ...
    struct inv_helper
    {
        ...
        )
        {
            ...
            COMPILE_TIME_ASSERT(matrix_exp<EXP>::NR == matrix_exp<EXP>::NCDLIB || 
                                matrix_exp<EXP>::NR == 0 ||
                                matrix_exp<EXP>::NCDLIB == 0);
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_diag_inv
    {
        ...
        const static long NR = ((M::NCDLIB!=0)&&(M::NR!=0))? (tmax<M::NR,M::NCDLIB>::value) : (0);
        const static long NCDLIB = NR;
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT(NCDLIB == 0 || uNC == 0 || NCDLIB == uNC);
        COMPILE_TIME_ASSERT(NCDLIB == 0 || wN == 0 || NCDLIB == wN);
        COMPILE_TIME_ASSERT(NCDLIB == 0 || vN == 0 || NCDLIB == vN);
        COMPILE_TIME_ASSERT(wX == 0 || wX == 1);

#ifdef DLIB_USE_LAPACK
        // use LAPACK but only if it isn't a really small matrix we are taking the SVD of.
        if (NR*NCDLIB == 0 || NR*NCDLIB > 3*3)
        {
...
// ----------------------------------------------------------------------------------------
    ...
    template <
        typename EXP
        >
    const matrix<typename EXP::type,EXP::NCDLIB,EXP::NR,typename EXP::mem_manager_type> pinv_helper ( 
        const matrix_exp<EXP>& m,
        double tol
    )
    /*!
    ...
    { 
        ...
        matrix<typename EXP::type, EXP::NCDLIB, EXP::NCDLIB,MM1, layout_type > v;

        typedef typename matrix_exp<EXP>::type T;

        matrix<T,matrix_exp<EXP>::NCDLIB,1,MM1, layout_type> w;
    ...
    template <
        typename EXP
        >
    const matrix<typename EXP::type,EXP::NCDLIB,EXP::NR,typename EXP::mem_manager_type> pinv ( 
        const matrix_exp<EXP>& m,
        double tol = 0
    )
    { 
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        matrix<T,matrix_exp<EXP>::NCDLIB,1,MM1, L1> W;
        svd3(m,u,W,v);
        w = diagm(W);
    }
