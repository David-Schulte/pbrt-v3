...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT(EXP1::NCDLIB == EXP2::NCDLIB || EXP1::NCDLIB == 0 || EXP2::NCDLIB == 0);
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_trans 
    {
        ...
        const static long NR = M::NCDLIB;
        const static long NCDLIB = M::NR;
...
// I introduced this struct because it avoids an inane compiler warning from gcc
    template <typename EXP>
    struct is_not_ct_vector{ static const bool value = (EXP::NR != 1 && EXP::NCDLIB != 1); };
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT(EXP1::NR*EXP1::NCDLIB == 0 ||
                            EXP2::NR*EXP2::NCDLIB == 0);
    ...
    typename enable_if_c<EXP1::NR == 1 && EXP2::NR == 1 && EXP1::NCDLIB != 1 && EXP2::NCDLIB != 1, typename EXP1::type>::type 
    ...
    template < typename EXP1, typename EXP2 >
    typename enable_if_c<EXP1::NR == 1 && EXP2::NCDLIB == 1 && EXP1::NCDLIB != 1 && EXP2::NR != 1, typename EXP1::type>::type 
    dot ( const matrix_exp<EXP1>& m1, const matrix_exp<EXP2>& m2) 
    { 
        DLIB_ASSERT(m1.size() == m2.size(), 
            "\t type dot(const matrix_exp& m1, const matrix_exp& m2)"
    ...
    template < typename EXP1, typename EXP2 >
    typename enable_if_c<EXP1::NCDLIB == 1 && EXP2::NR == 1 && EXP1::NR != 1 && EXP2::NCDLIB != 1, typename EXP1::type>::type 
    dot ( const matrix_exp<EXP1>& m1, const matrix_exp<EXP2>& m2) 
    { 
        DLIB_ASSERT(m1.size() == m2.size(), 
            "\t type dot(const matrix_exp& m1, const matrix_exp& m2)"
    ...
    template < typename EXP1, typename EXP2 >
    typename enable_if_c<EXP1::NCDLIB == 1 && EXP2::NCDLIB == 1 && EXP1::NR != 1 && EXP2::NR != 1, typename EXP1::type>::type 
    dot ( const matrix_exp<EXP1>& m1, const matrix_exp<EXP2>& m2) 
    { 
        DLIB_ASSERT(m1.size() == m2.size(), 
            "\t type dot(const matrix_exp& m1, const matrix_exp& m2)"
    ...
    template < typename EXP1, typename EXP2 >
    typename enable_if_c<(EXP1::NCDLIB*EXP1::NR == 1) || (EXP2::NCDLIB*EXP2::NR == 1), typename EXP1::type>::type 
    dot ( const matrix_exp<EXP1>& m1, const matrix_exp<EXP2>& m2) 
    { 
        DLIB_ASSERT(m1.size() == m2.size(), 
            "\t type dot(const matrix_exp& m1, const matrix_exp& m2)"
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_removerc 
    {
        ...
        const static long NCDLIB = (M::NCDLIB==0) ? 0 : (M::NCDLIB - 1);
        typedef typename M::type type;
        typedef typename M::const_ret_type const_ret_type;
        typedef typename M::mem_manager_type mem_manager_type;
        typedef typename M::layout_type layout_type;
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT((EXP::NCDLIB > C && C >= 0) || EXP::NR == 0);
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT((EXP::NCDLIB > C && C >= 0) || EXP::NCDLIB == 0);
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_remove_row 
    {
        ...
        const static long cost = M::cost+2;
        const static long NR = (M::NR==0) ? 0 : (M::NR - 1);
        const static long NCDLIB = M::NCDLIB;
        typedef typename M::type type;
        typedef typename M::const_ret_type const_ret_type;
        typedef typename M::mem_manager_type mem_manager_type;
        typedef typename M::layout_type layout_type;
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_diagm 
    {
        ...
        const static long cost = M::cost+2;
        const static long N = M::NCDLIB*M::NR;
        const static long NR = N;
        const static long NCDLIB = N;
        typedef typename M::type type;
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT(EXP::NR == 0 || EXP::NR == 1 || EXP::NCDLIB == 1 || EXP::NCDLIB == 0);
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_diag 
    {
        ...
        const static long cost = M::cost;
        const static long NR = tmin<M::NR,M::NCDLIB>::value;
        const static long NCDLIB = 1;
        typedef typename M::type type;
        typedef typename M::const_ret_type const_ret_type;
        typedef typename M::mem_manager_type mem_manager_type;
        typedef typename M::layout_type layout_type;
...
// this is a workaround for a bug in visual studio 7.1
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT(EXP::NCDLIB == 1 || EXP::NCDLIB == 0);

        // perform static checks to make sure the matrices contained in m are column vectors
        COMPILE_TIME_ASSERT(EXP::type::NCDLIB == 1 || EXP::type::NCDLIB == 0 );
...
// ----------------------------------------------------------------------------------------
    ...
    template <
        ...
        long NCDLIB
        >
    const matrix_op<op_uniform_matrix_2<T,NR,NCDLIB> > uniform_matrix (
        const T& val
    )
    {
        COMPILE_TIME_ASSERT(NR > 0 && NCDLIB > 0);

        typedef op_uniform_matrix_2<T,NR,NCDLIB> op;
...
// ----------------------------------------------------------------------------------------
    ...
    template <
        ...
        T val
        >
    const matrix_op<op_uniform_matrix<T,NR,NCDLIB,val> > uniform_matrix (
    )
    {
        COMPILE_TIME_ASSERT(NR > 0 && NCDLIB > 0);
        typedef op_uniform_matrix<T,NR,NCDLIB,val> op;
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT(EXP1::NR == EXP2::NR || EXP1::NR == 0 || EXP2::NR == 0);
        COMPILE_TIME_ASSERT(EXP1::NCDLIB == EXP2::NCDLIB || EXP1::NR == 0 || EXP2::NCDLIB == 0);
        COMPILE_TIME_ASSERT(EXP2::NR == EXP3::NR || EXP2::NR == 0 || EXP3::NR == 0);
        COMPILE_TIME_ASSERT(EXP2::NCDLIB == EXP3::NCDLIB || EXP2::NCDLIB == 0 || EXP3::NCDLIB == 0);
        DLIB_ASSERT(a.nr() == b.nr() &&
               a.nc() == b.nc() &&
               b.nr() == c.nr() &&
               b.nc() == c.nc(), 
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT(EXP3::NCDLIB == EXP4::NCDLIB || EXP3::NCDLIB == 0 || EXP4::NCDLIB == 0);
        DLIB_ASSERT(a.nr() == b.nr() &&
               a.nc() == b.nc() &&
               b.nr() == c.nr() &&
               b.nc() == c.nc() &&
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT(matrix_exp<EXP>::NCDLIB == 1);
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_scale_columns  
    {
        ...
        const static long NCDLIB = M1::NCDLIB;
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT((EXP2::NCDLIB == 1 || EXP2::NCDLIB == 0) || (EXP2::NR == 1 || EXP2::NR == 0));

        // figure out the compile time known length of v
        const long v_len = ((EXP2::NR)*(EXP2::NCDLIB) == 0)? 0 : (tmax<EXP2::NR,EXP2::NCDLIB>::value);

        // the length of v must match the number of columns in m
        COMPILE_TIME_ASSERT(EXP1::NCDLIB == v_len || EXP1::NCDLIB == 0 || v_len == 0);
...
// ----------------------------------------------------------------------------------------
    ...
    */
    ...
    struct op_diag_m_diag  
    {
        ...
        const static long NCDLIB = M2::NCDLIB;
    ...
    )
    {
        ...
        // figure out the compile time known length of d
        const long v_len = ((EXP1::NR)*(EXP1::NCDLIB) == 0)? 0 : (tmax<EXP1::NR,EXP1::NCDLIB>::value);
    ...
    )
    {
        ...
        // figure out the compile time known length of d
        const long v_len = ((EXP3::NR)*(EXP3::NCDLIB) == 0)? 0 : (tmax<EXP3::NR,EXP3::NCDLIB>::value);

        // the length of d must match the number of columns in m
        COMPILE_TIME_ASSERT(EXP2::NCDLIB == v_len || EXP2::NCDLIB == 0 || v_len == 0);
...
// ----------------------------------------------------------------------------------------
    ...
    template <
        typename T, long NR, long NCDLIB, typename mm, typename l1,
        long NR2, long NC2, typename mm2, typename l2
        >
    void sort_columns (
        matrix<T,NR,NCDLIB,mm,l1>& m,
        matrix<T,NR2,NC2,mm2,l2>& v
    )
    {
        COMPILE_TIME_ASSERT(NC2 == 1 || NC2 == 0);
        COMPILE_TIME_ASSERT(NCDLIB == NR2 || NCDLIB == 0 || NR2 == 0);
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_tensor_product 
    {
        ...
        const static long NCDLIB = M1::NCDLIB*M2::NCDLIB;
        typedef typename M1::type type;
        typedef const typename M1::type const_ret_type;
        typedef typename M1::mem_manager_type mem_manager_type;
        typedef typename M1::layout_type layout_type;
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_linpiece  
    {
        ...
        const static long NR = (M::NR*M::NCDLIB==0) ? (0) : (M::NR*M::NCDLIB-1); 
        const static long NCDLIB = 1; 
        typedef typename M::type type;
        typedef default_memory_manager mem_manager_type;
        typedef row_major_layout layout_type;
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_mat_to_vect 
    {
        ...
        const static long cost = M::cost+2;
        const static long NR = M::NCDLIB*M::NR;
        const static long NCDLIB = 1;
        typedef typename M::type type;
        typedef typename M::const_ret_type const_ret_type;
        typedef typename M::mem_manager_type mem_manager_type;
...
// ----------------------------------------------------------------------------------------
    ...
    template <
        ...
        typename MM
        >
    const matrix_op<op_mat_to_vect2<T,NR,NCDLIB,MM> > reshape_to_column_vector (
        const matrix<T,NR,NCDLIB,MM,row_major_layout>& m
    )
    {
        typedef op_mat_to_vect2<T,NR,NCDLIB,MM> op;
        return matrix_op<op>(op(m.ref()));
    }
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_join_rows 
    {
        ...
        const static long NCDLIB = (M1::NCDLIB*M2::NCDLIB != 0)? (M1::NCDLIB+M2::NCDLIB) : (0);
        typedef typename M1::type type;
        typedef typename M1::mem_manager_type mem_manager_type;
        typedef typename M1::layout_type layout_type;
...
// ----------------------------------------------------------------------------------------
    ...
    struct op_join_cols 
    {
        ...
        const static long cost = M1::cost + M2::cost + 1;
        const static long NCDLIB = tmax<M1::NCDLIB, M2::NCDLIB>::value;
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT(EXP1::NCDLIB == EXP2::NCDLIB || (EXP1::NCDLIB*EXP2::NCDLIB == 0));
...
// ----------------------------------------------------------------------------------------
    ...
    template <typename T, long NR, long NCDLIB, typename MM, typename L>
    uint32 hash (
        const matrix<T,NR,NCDLIB,MM,L>& item,
        uint32 seed = 0
    )
    {
