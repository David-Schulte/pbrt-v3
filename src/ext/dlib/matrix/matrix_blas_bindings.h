...
namespace dlib
{
    ...
    // Helpers for determining the data pointer, LDA, and incX arguments to BLAS functions.
        ...
        template <typename T, long NR, long NCDLIB, typename MM>
        int get_ld (const matrix<T,NR,NCDLIB,MM,row_major_layout>& m) { return m.nc(); }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_ld (const matrix<T,NR,NCDLIB,MM,column_major_layout>& m) { return m.nr(); }


        template <typename T, long NR, long NCDLIB, typename MM>
        int get_ld (const matrix_op<op_subm<matrix<T,NR,NCDLIB,MM,row_major_layout> > >& m) { return m.op.m.nc(); }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_ld (const matrix_op<op_subm<matrix<T,NR,NCDLIB,MM,column_major_layout> > >& m) { return m.op.m.nr(); }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_ld (const assignable_sub_matrix<T,NR,NCDLIB,MM,row_major_layout>& m) { return m.m.nc(); }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_ld (const assignable_sub_matrix<T,NR,NCDLIB,MM,column_major_layout>& m) { return m.m.nr(); }
        ...
        int get_inc (const matrix<T,NR,NCDLIB,MM,L>& ) { return 1; }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc (const matrix_op<op_subm<matrix<T,NR,NCDLIB,MM,row_major_layout> > >& m) 
        { 
        ...
        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc (const matrix_op<op_subm<matrix<T,NR,NCDLIB,MM,column_major_layout> > >& m) 
        { 
            if (m.nr() < m.op.m.nr())
                return 0;
            else
        ...
        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc (const assignable_sub_matrix<T,NR,NCDLIB,MM,row_major_layout>& m) 
        { 
            if (m.nc() < m.m.nc())
                return 0;
            else
        ...
        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc (const assignable_sub_matrix<T,NR,NCDLIB,MM,column_major_layout>& m) 
        {
            if (m.nr() < m.m.nr())
                return 0;
            else
        ...
        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const matrix_op<op_colm<matrix<T,NR,NCDLIB,MM,row_major_layout> > >& m)
        {
            return m.op.m.nc();
        }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const matrix_op<op_rowm<matrix<T,NR,NCDLIB,MM,row_major_layout> > >& )
        {
            return 1;
        }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const matrix_op<op_colm2<matrix<T,NR,NCDLIB,MM,row_major_layout> > >& m)
        {
            return m.op.m.nc();
        }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const matrix_op<op_rowm2<matrix<T,NR,NCDLIB,MM,row_major_layout> > >& )
        {
            return 1;
        }
        ...
        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const matrix_op<op_colm<matrix<T,NR,NCDLIB,MM,column_major_layout> > >& )
        {
            return 1;
        }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const matrix_op<op_rowm<matrix<T,NR,NCDLIB,MM,column_major_layout> > >& m)
        {
            return m.op.m.nr();
        }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const matrix_op<op_colm2<matrix<T,NR,NCDLIB,MM,column_major_layout> > >& )
        {
            return 1;
        }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const matrix_op<op_rowm2<matrix<T,NR,NCDLIB,MM,column_major_layout> > >& m)
        {
            return m.op.m.nr();
        }
        ...
        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const assignable_row_matrix<T,NR,NCDLIB,MM,row_major_layout>& )
        {
            return 1;
        }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const assignable_row_matrix<T,NR,NCDLIB,MM,column_major_layout>& m)
        {
            return m.m.nr();
        }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const assignable_col_matrix<T,NR,NCDLIB,MM,row_major_layout>& m)
        {
            return m.m.nc();
        }

        template <typename T, long NR, long NCDLIB, typename MM>
        int get_inc(const assignable_col_matrix<T,NR,NCDLIB,MM,column_major_layout>& )
        {
            return 1;
        }
        ...
        template <typename T, long NR, long NCDLIB, typename MM, typename L>
        const T* get_ptr (const matrix<T,NR,NCDLIB,MM,L>& m) { return &m(0,0); }

        template <typename T, long NR, long NCDLIB, typename MM, typename L>
        T* get_ptr (matrix<T,NR,NCDLIB,MM,L>& m) { return &m(0,0); }

        template <typename T, long NR, long NCDLIB, typename MM, typename L>
        const T* get_ptr (const matrix_op<op_subm<matrix<T,NR,NCDLIB,MM,L> > >& m) { return &m.op.m(m.op.r_,m.op.c_); }

        template <typename T, long NR, long NCDLIB, typename MM, typename L>
        const T* get_ptr (const matrix_op<op_colm<matrix<T,NR,NCDLIB,MM,L> > >& m) { return &m.op.m(0,m.op.col); }

        template <typename T, long NR, long NCDLIB, typename MM, typename L>
        const T* get_ptr (const matrix_op<op_rowm<matrix<T,NR,NCDLIB,MM,L> > >& m) { return &m.op.m(m.op.row,0); }

        template <typename T, long NR, long NCDLIB, typename MM, typename L>
        const T* get_ptr (const matrix_op<op_colm2<matrix<T,NR,NCDLIB,MM,L> > >& m) { return &m.op.m(0,m.op.col); }

        template <typename T, long NR, long NCDLIB, typename MM, typename L>
        const T* get_ptr (const matrix_op<op_rowm2<matrix<T,NR,NCDLIB,MM,L> > >& m) { return &m.op.m(m.op.row,0); }


        template <typename T, long NR, long NCDLIB, typename MM, typename L>
        T* get_ptr (assignable_col_matrix<T,NR,NCDLIB,MM,L>& m) { return &m(0,0); }

        template <typename T, long NR, long NCDLIB, typename MM, typename L>
        T* get_ptr (assignable_row_matrix<T,NR,NCDLIB,MM,L>& m) { return &m(0,0); }

        template <typename T, long NR, long NCDLIB, typename MM, typename L>
        T* get_ptr (assignable_sub_matrix<T,NR,NCDLIB,MM,L>& m) { return &m(0,0); }
