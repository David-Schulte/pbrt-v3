...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        /*!
            requires
                - NR == 1 || NCDLIB == 1 (i.e. this must be a column or row vector)
                - length >= 0 
                - if (NR == 1 && NCDLIB > 0) then
                    - length == NCDLIB
                - if (NCDLIB == 1 && NR > 0) then
                    - length == NR
            ensures
                - #*this is properly initialized
                - #aliases(*this) == true
        ...
        /*!
            requires
                - NR != 0 && NCDLIB != 0 (i.e. you can only use this constructor on statically sized matrices)
        ...
        /*!
            requires
                - This matrix is capable of having a size() == l.size().  Therefore, if
                  NR*NCDLIB != 0 then l.size() must equal NR*NCDLIB.  Alternatively, if NR or NCDLIB is
                  != 0 then l.size() must be a multiple of the non-zero NR or NCDLIB.
            ensures
                - #size() == l.size()
