...
// ----------------------------------------------------------------------------------------
    ...
    // We want to return the compile time constant if our NR and NCDLIB dimensions
    ...
    template < typename exp_type, long NCDLIB >
    struct get_nc_helper
    {
        static inline long get(const exp_type&) { return NCDLIB; }
    };
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        const static long NCDLIB = matrix_traits<EXP>::NCDLIB;
        const static long cost = matrix_traits<EXP>::cost;

        typedef matrix<type,NR,NCDLIB,mem_manager_type,layout_type> matrix_type;
        ...
        long nc (
        ) const { return get_nc_helper<exp_type,NCDLIB>::get(ref()); }
