...
// ----------------------------------------------------------------------------------------
    ...
    struct matrix_traits<matrix_multiply_exp<LHS,RHS> >
    {
        ...
        const static long NCDLIB = RHS::NCDLIB;
        ...
        const static bool lhs_is_costly = (LHS::cost > 2)&&(RHS::NCDLIB != 1 || LHS::cost >= 10000);
        const static bool rhs_is_costly = (RHS::cost > 2)&&(LHS::NR != 1 || RHS::cost >= 10000);
#else
        const static bool lhs_is_costly = (LHS::cost > 4)&&(RHS::NCDLIB != 1);
        ...
        const static long cost = ((tmax<LHS::NCDLIB,RHS::NR>::value!=0)? ((lhs_cost+rhs_cost)*tmax<LHS::NCDLIB,RHS::NR>::value):(10000));
    };
    ...
    public:
        ...
        const static long NCDLIB = matrix_traits<matrix_multiply_exp>::NCDLIB;
            ...
            rhs(rhs_)
        {
            ...
            COMPILE_TIME_ASSERT(LHS::NCDLIB == RHS::NR || LHS::NCDLIB*RHS::NR == 0);
...
// ----------------------------------------------------------------------------------------
    ...
    struct matrix_traits<matrix_add_exp<LHS,RHS> >
    {
        ...
        const static long NCDLIB = (RHS::NCDLIB > LHS::NCDLIB) ? RHS::NCDLIB : LHS::NCDLIB;
    ...
    public:
        ...
        const static long NCDLIB = matrix_traits<matrix_add_exp>::NCDLIB;
            ...
            rhs(rhs_)
        {
            ...
            COMPILE_TIME_ASSERT(LHS::NCDLIB == RHS::NCDLIB || LHS::NCDLIB == 0 || RHS::NCDLIB == 0);
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        const static long NCDLIB = matrix_traits<matrix_subtract_exp>::NCDLIB;
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        const static long NCDLIB = matrix_traits<matrix_div_scal_exp>::NCDLIB;
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        const static long NCDLIB = matrix_traits<matrix_mul_scal_exp>::NCDLIB;
...
// ----------------------------------------------------------------------------------------
    ...
    struct matrix_traits<matrix<T,num_rows, num_cols, mem_manager, layout> >
    {
        ...
        const static long NCDLIB = num_cols;
    ...
    public:
        ...
        const static long NCDLIB = matrix_traits<matrix>::NCDLIB;
        ...
        ) 
        {
            ...
            // row vector.
            COMPILE_TIME_ASSERT(NR == 1 || NCDLIB == 1);
            ...
            if (NR == 1)
            {
                DLIB_ASSERT(NCDLIB == 0 || NCDLIB == length,
                    "\tmatrix::matrix(length)"
                    << "\n\tSince this is a statically sized matrix length must equal NCDLIB"
                    << "\n\tlength: " << length 
                    << "\n\tNR:     " << NR 
                    << "\n\tNC:     " << NCDLIB 
                    << "\n\tthis:   " << this
        ...
        {
            DLIB_ASSERT( (NR == 0 || NR == rows) && ( NCDLIB == 0 || NCDLIB == cols) && 
                    rows >= 0 && cols >= 0, 
        ...
        )
        {
            ...
            COMPILE_TIME_ASSERT(EXP::NCDLIB == NCDLIB || NCDLIB == 0 || EXP::NCDLIB == 0);
            DLIB_ASSERT((NR == 0 || NR == m.nr()) && (NCDLIB == 0 || NCDLIB == m.nc()), 
        ...
        matrix(const std::initializer_list<T>& l)
        {
            ...
            else if (NCDLIB!=0) 
            {
                ...
                if (l.size() != 0)
                    data.set_size(l.size()/NCDLIB, NCDLIB);
            }
            else if (l.size() != 0)
            {
        ...
        {
            COMPILE_TIME_ASSERT(NR*NCDLIB == len && len > 0);
            size_t idx = 0;
            for (long r = 0; r < NR; ++r)
            {
                for (long c = 0; c < NCDLIB; ++c)
                {
        ...
        ) 
        {
            ...
            COMPILE_TIME_ASSERT(NCDLIB == 1 || NCDLIB == 0 || NR == 1 || NR == 0);
        ...
        {
            COMPILE_TIME_ASSERT(NCDLIB == 1 || NCDLIB == 0);
    ...
    private:
        ...
        typename layout::template layout<T,NR,NCDLIB,mem_manager> data;
    };
...
// ----------------------------------------------------------------------------------------
    ...
    template <
        ...
        >
    void swap(
        matrix<T,NR,NCDLIB,mm,l>& a,
        matrix<T,NR,NCDLIB,mm,l>& b
    ) { a.swap(b); }
...
// ----------------------------------------------------------------------------------------
    ...
    struct matrix_traits<const_temp_matrix<EXP> >
    {
        ...
        const static long NCDLIB = EXP::NCDLIB;
        const static long cost = 1;
    };
    ...
    public:
        ...
        const static long NCDLIB = matrix_traits<const_temp_matrix>::NCDLIB;
