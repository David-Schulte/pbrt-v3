...
// ----------------------------------------------------------------------------------------
    ...
    namespace ma
    {
        ...
        struct is_small_matrix<EXP, typename enable_if_c<EXP::NR>=1 && EXP::NCDLIB>=1 &&
        EXP::NR<=17 && EXP::NCDLIB<=17 && (EXP::cost <= 70)>::type> { static const bool value = true; };
        ...
        template < typename EXP >
        struct is_very_small_matrix<EXP, typename enable_if_c<EXP::NR>=1 && EXP::NCDLIB>=1 &&
        (mul<EXP::NR,EXP::NCDLIB>::value <= 16) && (EXP::cost <= 70)>::type> { static const bool value = true; };
...
// this code is here to perform an unrolled version of the matrix_assign() function
    template < typename DEST, typename SRC, long NR, long NCDLIB,
    ...
    struct matrix_unroll_helper
    {
        ...
        {
            dest(R,C) = src(R,C);
            matrix_unroll_helper<DEST,SRC,NR,NCDLIB, R + (C+1)/NCDLIB,  (C+1)%NCDLIB>::go(dest,src);
        }
    };

    template < typename DEST, typename SRC, long NR, long NCDLIB, long R, long C >
    struct matrix_unroll_helper<DEST,SRC,NR,NCDLIB,R,C,true>
    ...
    !*/
    {
        COMPILE_TIME_ASSERT(SRC::NR*SRC::NCDLIB != 0);
        matrix_unroll_helper<DEST,SRC, SRC::NR, SRC::NCDLIB>::go(dest,src);
    }
